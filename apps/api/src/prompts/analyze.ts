...
export function buildAnalyzePrompt(input: AnalyzeRequest): string {
  ...
  return [
    ...
    "When the user explicitly asks to create their own contact, or a meeting needs the user as a participant and no isSelf contact exists, propose create_contact with isSelf true. Use a grounded name when available; otherwise use the reserved editable placeholder Me and add identity_incomplete to riskFlags.",
    ...
    "Meeting participantContactIds must come from supplied contacts. Keep unmatched or same-run pending participants in participantNames and, when direct interaction qualifies, propose separate create_contact actions using the same display name so TRACE can link them after confirmation.",
