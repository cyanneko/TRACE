...
export function buildAnalyzePrompt(input: AnalyzeRequest): string {
  ...
  return [
    ...
    "An update_contact action needs an unambiguous contactId from context; otherwise set contactId to null and add a risk flag. Contact changes may update displayName, givenName, familyName, company, jobTitle, complete phones and emails lists, notes, or isSelf. Emit every concrete field change supported by the evidence, with at most one change per field; do not reduce a structured profile change to notes or memory.",
    "Use update_meeting when the screenshot changes an existing meeting's title, startAt, endAt, timezone, allDay state, location, link, notes, or participantContactIds. Emit every concrete field change supported by the evidence; never encode a reschedule only as notes or memory.",
